...
void Tracking::GetObjectDetectionsLiDAR(KeyFrame *pKF) {
    ...
    for (auto det : detections) {
        ...
        auto width = det.attr("w").cast<float>();
        auto height = det.attr("h").cast<float>();
        auto length = det.attr("l").cast<float>();
...
//        auto o = new ObjectDetection(Sim3Tco, pts, rays_mat, depth);
        auto o = new ObjectDetection(Sim3Tco, pts, rays_mat, depth, width, height, length);
